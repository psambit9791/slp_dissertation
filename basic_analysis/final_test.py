...
DATAPATH = "../emotion_bert/data/balanced_emotion/"

train = pd.read_csv(DATAPATH+"train.txt", sep="\t", header=None, names=["emotion", "current_dialog"])
# train = train.drop(columns=["act"])

valid = pd.read_csv(DATAPATH+"validation.txt", sep="\t", header=None, names=["emotion", "current_dialog"])
# valid = valid.drop(columns=["act"])

test = pd.read_csv(DATAPATH+"test.txt", sep="\t", header=None, names=["emotion", "current_dialog"])
