...
python $FPPATH/train.py --cuda -o $FPPATH/fp_lj_sentiment/ --log-file $FPPATH/output/nvlog_fp_lj_sentiment.json --dataset-path $LJPATH --training-files $LJPATH/filelists/train_filelist.txt --validation-files $LJPATH/filelists/val_filelist.txt --pitch-mean-std-file $LJPATH/pitch_phone_stats__transcript.json --data-inputs-file $LJPATH/sentiment_fp_data.json --model-conditions Phon-lj-ctxt-utt --epochs 500 --optimizer lamb -lr 0.1 -bs 32